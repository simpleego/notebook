<script>
	const coffeeOptions = [
		...
		{ name: '설탕커피', image: '/images/sugarOut.png', price: '200원' },
		{ name: '블랙커피', image: '/images/blackOut.png', price: '100원' }
	];
</script>
	...
	<div class="menu">
		{#each coffeeOptions as coffee}
			<div class="option">
				...
				<button class="coffee-button" on:click={() => selectCoffee(coffee.name)}>
					{coffee.name}
				</button>
			</div>
		{/each}
